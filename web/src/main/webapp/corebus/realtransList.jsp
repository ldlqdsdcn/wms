...
<title><delmar:message key="realtrans.title" /></title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
</head>

<body >

<s:form action="realtrans_list" namespace="/corebus"  theme="simple" >
...
<!-- <c:out value="${buttons}" escapeXml="false"/> -->
 <display:table name="sessionScope.MAP_KEY_OF_SESSION.realTransList" cellspacing="0" cellpadding="0"  requestURI=""
		...
		<display:column  titleKey="common.label.sequence" media="html csv excel xml pdf rtf">
		     ...
		     <display:column   titleKey="realtrans.column.HawbBillNo" sortable="true"   media="html">
		      <a href="javascript:viewExport('<c:out value="${list.id}"/>')"><c:out value="${list.hawbbillno}"/>
     		  </a>
		     </display:column>
		     <display:column property="businessno"  escapeXml="true"  titleKey="realtrans.column.businessno" sortable="true" media="csv excel xml pdf rtf"></display:column>	
		    <display:column property="flight"  escapeXml="true"  titleKey="realtrans.column.flight" sortable="true" />	
		    ...
		    <display:column property="flightdate"  escapeXml="true"  titleKey="customer.column.created"  sortable="true"  decorator="com.delmar.core.web.displaytag.decorator.DateDecorator"/>
...
<script type="text/javascript">
    function viewExport(id) {
       window.location='<c:url value="/corebus/realtrans_edit.action"/>?id='+id;
    }
