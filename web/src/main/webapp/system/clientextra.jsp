...
<div id="clientextra_form" title="<delmar:message key='clientextra.dialog.title'/>" >
  ...
  <form class="dialog-form" >
    <fieldset>
		...
		<s:label for="propId"  value="%{#session.resource.get('clientextra.column.propid')}" />
		<s:select list="extraPropList" listKey="datadictId" listValue="name"  name="extraPropId" id="extraPropId" style="width:200px"></s:select>
		<p>
		<s:label for="extraValue"  value="%{#session.resource.get('clientextra.column.value')}" />
		<s:textfield name="extraValue" id="extraValue"/>
		<p>
		<s:label for="extraRemark"  value="%{#session.resource.get('clientextra.column.remark')}" />
